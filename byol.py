...
#### This code has been adapted and modified. #########
#  The original source code is from https://github.com/lucidrains/byol-pytorch/blob/master/byol_pytorch/byol_pytorch.py
#######################################################
...
def MLP(dim, projection_size, hidden_size=4096, proj_head_type='2layer'):
    projector = None
    if proj_head_type == '2layer':
      projector = nn.Sequential(nn.Linear(dim, hidden_size, bias=False),
...
def SimSiamMLP(dim, projection_size, hidden_size=4096, proj_head_type='2layer'):
    # NOTE: 'proj_head_type' is not used in this function.
    # ('proj_head_type' is used in MLP though)
...
class NetWrapper(nn.Module):
    def __init__(self, net, projection_size, projection_hidden_size, layer = -2, use_simsiam_mlp = False,
        ...
        self.proj_head_type = proj_head_type
    ...
    def _get_projector(self, hidden):
        ...
        projector = create_mlp_fn(dim, self.projection_size, self.projection_hidden_size, proj_head_type=self.proj_head_type)
...
class BYOL(nn.Module):
    ...
    def forward(
        ...
        if return_embedding:
            return self.online_encoder(x1, return_projection = False),  self.online_encoder(x2, return_projection = False)
        
        if return_projection:
            return self.online_encoder(x1, return_projection = True)[0],  self.online_encoder(x2, return_projection = True)[0]
        ...
        if return_prediction:
            return online_pred_one, online_pred_two
